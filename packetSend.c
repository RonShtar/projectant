...
{
    sock_raw=socket(AF_PACKET,SOCK_RAW,IPPROTO_RAW); //send ipv4 packets 
    if(sock_raw == -1)
    {
...
void get_eth_index() //get network interface index
{
	memset(&ifreq_i,0,sizeof(ifreq_i));
	strncpy(ifreq_i.ifr_name,"enp0s3",IFNAMSIZ-1); //copy network interface name in order to make system call
	if((ioctl(sock_raw,SIOCGIFINDEX,&ifreq_i))<0) //system call to get network interface index and put it in ifreq struct
		printf("error in index ioctl reading");
}
char* get_mac(char* sendbuff) //make data link header and then returns updated packet
{
	memset(&ifreq_c,0,sizeof(ifreq_c));
	strncpy(ifreq_c.ifr_name,"enp0s3",IFNAMSIZ-1); //copy network interface name in order to make system call
...
char* get_ip(char *dataToSend,char* sendbuff) //make ip header and returns updated packet
{
	memset(&ifreq_ip,0,sizeof(ifreq_ip));
	strncpy(ifreq_ip.ifr_name,"enp0s3",IFNAMSIZ-1); //copy network interface name in order to make system call
...
void build_packet(char* dataToSend,char* sendbuff) //build packet headers
{
    get_eth_index(sendbuff);  //get network interface number
	sendbuff=get_mac(sendbuff); //make data link header
	sendbuff=get_ip(dataToSend,sendbuff); //make ip header 
...
int sendPacket(char* data)
{
    ...
    if(init_rawsocket() == 1) //init raw socket to send packet
    {
        build_packet(data,sendbuff); //build packet and then send it
    }
}
