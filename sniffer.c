...
static void printEth(struct ethHead *eth_p)
{
    printf("\nEthernet Header\n");
...
static void printIp(struct ipHead *ip_p)
{
    printf("\nIP Header\n");
    printf("\t|-Version %d\n", ip_p->version);
    ...
    printf("\t|-Type Of Service : %d\n", ip_p->tos);                // print tos field,because its only byte varriable we can print it same we got it.
    printf("\t|-Total Length : %d Bytes\n", ntohs(ip_p->totalLen)); // print ipv4 header+payload length in host form
    printf("\t|-Identification : %d\n", ntohs(ip_p->id));
    printf("\t|-Time To Live : %d\n", ip_p->ttl);  // print ttl field,because its only byte varriable we can print it same we got it.
    printf("\t|-Protocol : %d\n", ip_p->protocol); // print protocol field,because its only byte varriable we can print it same we got it.
...
static void printTcp(struct tcpHead *tcp_p)
{
    printf("\nTCP Header\n");
...
static void printUdp(struct udpHead *udp_p) // print all udp header fields
{
    printf("\nUDP Header\n");
    ...
    printf("\t|-UDP Checksum : %d\n", ntohs(udp_p->checksum));   // print checksum in host form
}
static void printIcmp(struct icmpHead *icmp_p)
{
    printf("\nICMP Header\n");
    printf("\t|-Type : %d\n", icmp_p->type); 
    printf("\t|-Code : %d\n", icmp_p->code); 
